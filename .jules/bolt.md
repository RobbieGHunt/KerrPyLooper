...
## 2024-05-25 - ThreadPoolExecutor in PyQt5
**Learning:** Using `executor.map(..., list)` inside a PyQt5 thread eagerly evaluates all iterables and pushes tasks to the executor queue. If a user tries to abort, exiting the `with ThreadPoolExecutor` block calls `.shutdown(wait=True)`, which hangs the application until all queued tasks complete because they cannot be easily cancelled.
**Action:** When parallelizing within a PyQt5 GUI that needs an abort mechanism, use `executor.submit(...)` to collect futures. Then, iterate over futures, checking the abort flag. If abort is requested, manually call `.cancel()` on all remaining futures before exiting the context manager to prevent the UI thread from freezing.
