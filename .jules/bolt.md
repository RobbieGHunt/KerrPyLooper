...
Because image loading and array conversions via Pillow/Numpy are memory-bound processes wrapped under Python's GIL, applying `concurrent.futures.ThreadPoolExecutor` to parallelize iterations often blocks the GUI main thread and worsens performance. Without large structural refactoring, the existing `itertuples` and memory hoisting operations are optimally performant.
