...
## 2024-06-25 - Fast SQDIFF template matching via FFT convolution
**Learning:** O(N^2) nested loops for calculating Sum of Squared Differences (SQDIFF) across a search grid (such as in image template matching or drift correction) are a massive bottleneck.
**Action:** Expand the SQDIFF equation `(A-B)^2 = A^2 - 2AB + B^2` and use `scipy.signal.fftconvolve(..., mode='valid')` to calculate the components in O(N log N) frequency-domain time. This yielded a 10x speedup in `drift_corrector.py` without losing precision.
